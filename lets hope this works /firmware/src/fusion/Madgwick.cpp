...
static inline float clampf_m(float x, float lo, float hi) { return fmaxf(lo, fminf(hi, x)); }
...
float betaFromConf(bool still, bool useMag, float confA, float confM) {
  ...
  float base = still ? kBetaStill : kBetaMove;
  base *= clampf_m(confA, 0.10f, 1.0f);
  if (useMag) base *= clampf_m(confM, 0.10f, 1.0f);
  return clampf_m(base, 0.015f, 0.25f);
}
