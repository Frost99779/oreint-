...
// WARNING: this assumes a calibrated magnitude of ~1.0 (unit sphere). With the BMM150
// and this firmware's min/max soft-iron cal, calibrated magnitude is ~0.04 mT, so this
// returns ~0 for all samples. Safe only because it is unused: kEnableConfBeta is false
// and Madgwick normalizes the mag vector internally. Do NOT enable kEnableConfBeta
// without first rescaling this against the actual calibrated magnitude.
float magConfidence(float mx_mT, float my_mT, float mz_mT) {
    ...
    if (meas < 1e-6f) return 0.0f;
    const float rel  = fabsf(meas - 1.0f);
